...
4. **Build and deliver.** Generate the standalone HTML beside the source or in the requested output folder. Then give the user the absolute file link and a compact usage guide:
   - open the file on desktop; on iPhone/iPad, use **Microsoft Edge** for opening, selecting, annotating, and exporting. Safari and Chrome may not appear in the Files app's “Open with” menu for local `.html` files because of iOS file sandbox restrictions. Use **Share → Open in Edge** when available, or serve the folder from a local/hosted HTTP server and open the resulting `http(s)://` link in Edge;
   - switch 中文 / EN from the top bar;
   - use the left chapter rail to jump sections; desktop columns scroll independently;
   - select text for underline, highlighter, or a reflection note; on mobile, wait for the native selection handles/menu to finish before tapping the annotation toolbar. Clicking the same underline/highlighter action again removes that mark;
...
6. Save a standalone `.html` file next to the source or in the requested output folder. It must work with `file://` and have no remote JavaScript, CSS, fonts, images, or API calls. Inline small SVG icons if needed. For iOS/iPadOS, explicitly recommend Microsoft Edge as the supported browser for opening local HTML files, touch selection, annotation, and export; Safari/Chrome local-file handoff is not the supported path.
...
- Mobile selection compatibility: do not require the selection to begin and end in the same text node; touch selection commonly spans multiple nodes. Accept a range that stays inside one `.section`, use `Range.extractContents()` when wrapping a mark, and listen to `selectionchange` plus `touchend`/`pointerup` with a short delay. Keep the last valid range when iPad Edge temporarily collapses the native selection menu, and do not clear that range merely because `selectionchange` reports a transient collapse. On coarse-pointer devices, place the toolbar at the bottom with safe-area padding and let toolbar clicks use the saved range.
- Storage fallback: guard all `localStorage` access because some iOS `file://` contexts deny storage access. Fall back to an in-memory adapter so annotation actions still work; keep the explicit JSON Save/Restore flow as the reliable cross-browser recovery path.
